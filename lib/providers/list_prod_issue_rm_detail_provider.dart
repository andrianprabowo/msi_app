...
class ListProdIssueRmDetailProvider with ChangeNotifier {
  List<ListProdIssueRmDetail> _items = [];
  ListProdIssueRmDetail _selected;

  List<ListProdIssueRmDetail> get items => _items;
  ListProdIssueRmDetail get selected => _selected;

  Future<void> getAllData(int idGrpodlvHeader) async {
    final url = '$kBaseUrl/tgrpo/tgrpo/api/IssueRm/$idGrpodlvHeader';
    ...
    try {
     ...
     // print("data gajelas $_selected");
      ...
      final List<ListProdIssueRmDetail> list = [];
      data.forEach((map) {
        list.add(ListProdIssueRmDetail.fromMap(map));
      });
  ...
  void selectPo(ListProdIssueRmDetail purchaseOrder) {
