...
class ListPickListRtvDetailProvider with ChangeNotifier {
  List<ListPickListRtvDetail> _items = [];
  ListPickListRtvDetail _selected;

  List<ListPickListRtvDetail> get items => _items;
  ListPickListRtvDetail get selected => _selected;

  Future<void> getAllData(int idGrpodlvHeader) async {
    final url = '$kBaseUrl/tgrpo/tgrpo/api/rvpl/$idGrpodlvHeader';
    ...
    try {
     ...
     // print("data gajelas $_selected");
      ...
      final List<ListPickListRtvDetail> list = [];
      data.forEach((map) {
        list.add(ListPickListRtvDetail.fromMap(map));
      });
  ...
  void selectPo(ListPickListRtvDetail purchaseOrder) {
