...
class PickBatchSoProvider with ChangeNotifier {
  List<PickBatchSo> _items = [];
  ...
  List<PickBatchSo> get pickedItems {
  ...
  Future<void> getPlBatchByItemWhs(String itemCode, String binCode) async {
    ...
    try {
      ...
      final List<PickBatchSo> list = [];
      data.forEach((map) {
        list.add(PickBatchSo.fromMap(map));
      });
  ...
  PickBatchSo findByBatchNo(String batchNo) {
    return _items.firstWhere((item) => item.batchNo == batchNo);
  }

  void updatePickQty(String batchNo, double pickQty) {
    PickBatchSo item = _items.where((item) => item.batchNo == batchNo).first;
    if (item != null) {
