...
class ListGrpoDetailProvider with ChangeNotifier {
  List<ListGoodReceiptPoDetail> _items = [];
  ListGoodReceiptPoDetail _selected;

  List<ListGoodReceiptPoDetail> get items => _items;
  ListGoodReceiptPoDetail get selected => _selected;

  Future<void> getAllData(int idGrpoHeader) async {
    final url = '$kBaseUrl/tgrpo/tgrpo/api/grpo/$idGrpoHeader';
    ...
    try {
      ...
      final List<ListGoodReceiptPoDetail> list = [];
      data.forEach((map) {
        list.add(ListGoodReceiptPoDetail.fromMap(map));
      });
  ...
  void selectPo(ListGoodReceiptPoDetail purchaseOrder) {
