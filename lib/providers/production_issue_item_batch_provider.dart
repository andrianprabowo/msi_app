...
class ProductionIssueItemBatchProvider with ChangeNotifier {
  List<ProductionIssueItemBatchModel> _items = [];
  double _totalPicked = 0.0;

  List<ProductionIssueItemBatchModel> get items {
    return _items.where((item) => item.availableQty > item.putQty).toList();
  }

  List<ProductionIssueItemBatchModel> get pickedItems {
  ...
  Future<void> getBatchListByItemWarehouse(
    ...
    try {
      ...
      final List<ProductionIssueItemBatchModel> list = [];
      data.forEach((map) {
        list.add(ProductionIssueItemBatchModel.fromMap(map));
      });
  ...
  ProductionIssueItemBatchModel findByBatchNo(String batchNo) {
    return _items.firstWhere((element) => element.batchNo == batchNo);
  }

  void addBatchList(
    ProductionIssueItemModel itemBin,
  ...
  void inputQtyNonBatch(ProductionIssueItemModel itemBin, double qty) {
    itemBin.putQty = qty;
    notifyListeners();
  ...
  void updatePickQty(String batchNo, double putQty) {
    ProductionIssueItemBatchModel item = _items.where((item) => item.batchNo == batchNo).first;
    if (item != null) {
      item.putQty = putQty;
    }
