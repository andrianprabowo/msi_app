...
class ItemListInvDispatchSo extends StatelessWidget {
  final ListInvDispatchSo item;

  const ItemListInvDispatchSo(this.item);
...
Widget get textStatus {
  ...
  Widget build(BuildContext context) {
    final listProvider =
        Provider.of<ListInvDispatchSoProvider>(context, listen: false);
    return InkWell(
      onTap: () {
        if (item.status == 0) {
          listProvider.selectPo(item);
          Navigator.of(context)
              .pushNamed(ListInvDispatchSoDetailScreen.routeName, arguments: item);
        }
      },
      child: Container(
