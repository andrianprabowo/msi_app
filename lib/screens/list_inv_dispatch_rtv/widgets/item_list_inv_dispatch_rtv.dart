...
class ItemListInvDispatchRtv extends StatelessWidget {
  final ListInvDispatchRtv item;

  const ItemListInvDispatchRtv(this.item);
...
Widget get textStatus {
  ...
  Widget build(BuildContext context) {
    final listProvider =
        Provider.of<ListInvDispatchRtvProvider>(context, listen: false);
    return InkWell(
      onTap: () {
        if (item.status == 0) {
          listProvider.selectPo(item);
          Navigator.of(context)
              .pushNamed(ListInvDispatchRtvDetailScreen.routeName, arguments: item);
        }
      },
      child: Container(
